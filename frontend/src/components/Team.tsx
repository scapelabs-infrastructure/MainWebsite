            ...
            <div className="grid grid-cols-2 gap-3">
              {members.slice(0, 4).map((member, i) => (
                ...
                >
                  ...
                  {/* Gradient only covers bottom strip so faces stay visible */}
                  <div
                    className="absolute inset-0 pointer-events-none"
                    style={{ background: 'linear-gradient(180deg, transparent 55%, rgba(8,8,16,0.97) 100%)' }}
                  />
                  <div className="absolute bottom-0 left-0 right-0 p-3">
                    <p className="text-[#E8E8F0] font-semibold text-xs">{member.name}</p>
                    <p className="text-[#E8E8F0]/50 text-xs mt-0.5" style={{ fontSize: '10px' }}>{member.role}</p>
